...
export class CommandManager {
    ...
    for (const file of commandFiles) {
      try {
        ...
        if (command && command.name) {
          ...
          this.bot.on("login", () => {
            Logger.log(
              "info",
              "CommandManager",
              `Comando ${chalk.cyanBright(command.name)} foi carregado`,
            );
          });
        }
      } catch (error) {
        Logger.log(
          "error",
          "CommandManager",
          `Erro ao caregar o comando ${chalk.cyanBright(name)}: ${error}`,
        );
      }
    }
  }
  ...
  private executeCommand(name: string, args: string[]) {
    ...
    if (command) {
      ...
      Logger.log("error", "CommandManager", `Comando "${chalk.cyanBright(name)}" não encontado`);
    }
  }
